...
use NDM\Bundle\TryCatch\ApiBundle\Entity;
...
class ChannelTransformer extends CollectionTransformer {
	 ...
	 * {@inheritdoc}
	 *
	 * @see NDM\Bundle\TryCatch\ApiBundle\Ingester\Map\Transformer.CollectionTransformer::Transformer()
	 */
	...
	protected function writeChannel($channel, Entity\Component $component, array $record = array()) {
		...
		$channel = $this->om->getRepository('TryCatchApiBundle:Channel')->findOneBy(array('name' => trim($name)));
		if(!$channel) {
			// Otherwise create it
